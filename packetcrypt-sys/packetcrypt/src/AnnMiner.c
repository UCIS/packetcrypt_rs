...
static inline void setRequestedState(AnnMiner_t *ctx, Worker_t *w,
                                     enum ThreadState ts) {
  (void)(ctx);
  w->reqState = ts;
}
...
inline void merkle_flatten_item_row(int depth, uint8_t *out,
                                    const uint8_t *table, int itemSz) {
  int odx = 0;
...
static void *thread(void *vworker) {
  ...
  CryptoCycle_Item_t *item_cache = (CryptoCycle_Item_t *)malloc(
      Announce_TABLE_SZ * sizeof(CryptoCycle_Item_t));
  ...
  for (;;) {
    ...
    if (rebuild) {
      ...
      merkle_flatten_item_row(13, &merkle, (const uint8_t *)PrimaryTable,
                              sizeof *PrimaryTable);
    ...
    // Buf_OBJCPY(&worker->job.parentBlockHash,
    //            &worker->job.hah.hash.thirtytwos[0]);
...
#ifdef JIT_ENABLED
    rh_jit_program_t *program =
        rh_generate_program(vctx->progbuf, vctx->progLen);
#endif
...
#ifdef JIT_ENABLED
    rh_make_item(itemNo, &item_1, vctx, &cycle_program_seed[0].thirtytwos[1],
                 program);
#else
    Announce_mkitem2(itemNo, &item_1, &cycle_program_seed[0].thirtytwos[1],
                     vctx);
#endif
...
#ifdef JIT_ENABLED
          rh_make_item(itemNo, &item_n, vctx,
                       &cycle_program_seed[0].thirtytwos[1], program);
#else
          Announce_mkitem2(itemNo, &item_n,
                           &cycle_program_seed[0].thirtytwos[1], vctx);
#endif
...
AnnMiner_t *AnnMiner_create(uint32_t minerId, int threads, void *callback_ctx,
                            AnnMiner_Callback ann_found) {
  assert(threads);
  ...
  for (int i = 0; i < threads; i++) {
    ...
    assert(!pthread_create(&ctx->workers[i].thread, NULL, thread,
                           &ctx->workers[i]));
  }
  return ctx;
}
...
double AnnMiner_hashesPerSecond(AnnMiner_t *ctx) {
  ...
  uint64_t micros = ((uint64_t)tv.tv_sec - tv0.tv_sec) * 1000000ull +
